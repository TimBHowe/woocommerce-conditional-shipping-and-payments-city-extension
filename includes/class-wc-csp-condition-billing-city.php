<?php
/**
 * WC_CSP_Condition_Billing_City class
 *
 * @package  WooCommerce Conditional Shipping and Payments - City Extension
 * @since    1.3.0
 */
class WC_CSP_Condition_Billing_City extends WC_CSP_Package_Condition {

	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->id                             = 'billing_address';
		$this->title                          = __( 'Billing City/Suburb', 'woocommerce-conditional-billing-and-payments-city-extension' );
		$this->priority                       = 30;
	...
	public function get_condition_resolution( $data, $args ) {
		...
		return __( 'Enter a valid billing city/suburb', 'woocommerce-conditional-shipping-and-payments-city-extension' );
	}
	...
	public function check_condition( $data, $args ) {
		...
		// Else check the customer object.
			...
			if ( empty( $city ) ) {
				$is_matching_package = apply_filters( 'woocommerce_csp_billing_city_condition_match_empty_billing_address', $this->modifier_is( $data['modifier'], array( 'not-has' ) ), $data, $args );
	...
	public function get_admin_fields_html( $index, $condition_index, $condition_data ) {
		...
		<div class="condition_row_inner">
			...
			<div class="condition_value" style="<?php echo in_array( $modifier, $zero_config_modifiers ) ? 'display:none;' : ''; ?>">
				<input type="text"  name="restriction[<?php echo esc_attr( $index ); ?>][conditions][<?php echo esc_attr( $condition_index ); ?>][value]" value="<?php echo esc_attr( $city ); ?>" placeholder="<?php esc_attr_e( 'Enter city/suburb names', 'woocommerce-conditional-shipping-and-payments-city-extension' ); ?>" step="any" min="0"/>
				<span class="description"><?php esc_attr_e( 'Enter the cities/suburbs (case-insensitive), separated by pipe (|) to search for in the Billing City/Suburb field.', 'woocommerce-conditional-shipping-and-payments-city-extension' ); ?></span>
			</div>
